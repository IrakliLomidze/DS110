...
namespace ILG.DS.Notification.Visited
{
    public class DSVisitedNotificationManager
    {
        ...
        private bool _writeToFile_save()
        {
            ...
            string t_sourcefile = $"dsvisited_{app.state.DS_Profile_Id.Trim().ToLower()}_v.{DateTime.Now.Ticks}.json";
            string l_sourcefile = $"dsvisited_{app.state.DS_Profile_Id.Trim().ToLower()}_v.last.json";
            try
            {
