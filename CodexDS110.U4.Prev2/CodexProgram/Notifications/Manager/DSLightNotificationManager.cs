...
namespace ILG.DS.Notification
{
    public class DSLightNotificationManager
    {
        ...
        private bool _writeToFile_save()
        {
            ...
            string t_sourcefile = $"dsnotification_{app.state.DS_Profile_Id.Trim().ToLower()}_dsntf.{DateTime.Now.Ticks}.json";
            string l_sourcefile = $"dsnotification_{app.state.DS_Profile_Id.Trim().ToLower()}_dsntf.last.json";
            try
            {
        ...
        public void NewNotification(DSLightNotificationRecord record)
        {
            ...
            int max = _datacontexts.Select(n => n.nt_id).Max();
            record.nt_id = max+1;
