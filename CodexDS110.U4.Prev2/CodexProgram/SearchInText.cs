...
namespace ILG.Codex.CodexR4
{
    public partial class SearchInText : Form
    {
        ...
        private void ultraButton2_Click(object sender, EventArgs e)
        {
            ...
            if (findpostion == -1)
            {
                if (isfff == true) ILG.Windows.Forms.ILGMessageBox.Show("ტექსტში '" + cbSearchString.Text.Trim() + "' მეტი არ მოიძებნა ");
                else ILG.Windows.Forms.ILGMessageBox.Show("ტექსტში '" + cbSearchString.Text.Trim() + "' არ მოიძებნა ");
                isfff = false;
            }
            else
            {
